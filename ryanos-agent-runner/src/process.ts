...
export async function run(command: string, args: string[], cwd: string, timeoutMs = 300000) {
  return new Promise<{ stdout: string; stderr: string; code: number }>((resolve, reject) => {
    ...
    const child = spawn(executable, executableArgs, { cwd, shell: false, windowsHide: true, env: { PATH: process.env.PATH ?? "", PATHEXT: process.env.PATHEXT ?? ".COM;.EXE;.BAT;.CMD", SystemRoot: process.env.SystemRoot ?? "", TEMP: process.env.TEMP ?? "", TMP: process.env.TMP ?? "", ComSpec: process.env.ComSpec ?? "" } });
