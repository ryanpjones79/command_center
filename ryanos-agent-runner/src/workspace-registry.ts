...
export function resolveWorkspace(registry: Record<string, Workspace>, identifier: string, capability: Capability) {
  ...
  if (["RYKAS_OPERATIONS_READ", "RYKAS_OWNER_DATA_UPDATE", "RYKAS_AMAZON_TRUTH_REFRESH"].includes(capability)) {
    if (identifier !== "rykas-repo" || workspace.projectSlug !== "rykas") throw new Error("Rykas truth operations require the fixed rykas-repo workspace.");
    if (workspace.networkPolicy !== "LOCALHOST_ONLY" || workspace.sensitivity !== "STANDARD") throw new Error("Rykas truth operations require the standard localhost-only boundary.");
