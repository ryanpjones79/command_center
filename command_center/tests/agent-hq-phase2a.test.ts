...
describe("Rykas real-truth read boundary", () => {
  ...
  it("turns only a real purchase-ready row into an authorization-only bounded decision", async () => {
    ...
    const financialSnapshot = { capitalPlan: { status: "READY", safeBuyingCapacity: 500 } };
    const plan = await new DeterministicProjectManagerAgent().chooseNextWork({ profile: "RYKAS_GM", projectId, projectName: "Rykas", objective: "Profit", primaryKpi: null, currentBottleneck: "Decision", instructions: "Use truth", autonomyPolicy: "Read", escalationPolicy: "Purchases", existingWorkTitles: [], operatingMode: "LIVE_INTERNAL", toolEvidence: [{ toolId: "rykas.operations.snapshot", summary: "ready", output: { sourcingAllowed: true, realTruth: { stale: false, purchaseExecuted: false, data: { purchaseCandidates: [candidate], opportunities: [candidate], blockers: [], financialSnapshot } } } }] });
    expect(plan.ownerNeeded).toBe(true); expect(plan.ownerDecision).toMatchObject({ category: "PURCHASE_INVENTORY", availableChoices: ["BUY", "REDUCE", "PASS"], boundedPayload: { opportunityId: "US:B000000001", maximumSpend: 120, purchaseExecuted: false } });
    expect(plan.ownerDecision?.context).toContain('"expectedProfit":48');

    const constrained = await new DeterministicProjectManagerAgent().chooseNextWork({ profile: "RYKAS_GM", projectId, projectName: "Rykas", objective: "Profit", primaryKpi: null, currentBottleneck: "Decision", instructions: "Use truth", autonomyPolicy: "Read", escalationPolicy: "Purchases", existingWorkTitles: [], operatingMode: "LIVE_INTERNAL", toolEvidence: [{ toolId: "rykas.operations.snapshot", summary: "ready", output: { sourcingAllowed: true, realTruth: { stale: false, purchaseExecuted: false, data: { purchaseCandidates: [candidate], opportunities: [candidate], blockers: [], financialSnapshot: { capitalPlan: { status: "READY", safeBuyingCapacity: 75 } } } } } }] });
    expect(constrained.ownerDecision).toMatchObject({ recommendedChoice: "REDUCE", availableChoices: ["REDUCE", "PASS"], boundedPayload: { requestedSpend: 120, maximumSpend: 75, purchaseExecuted: false } });
  });
});
