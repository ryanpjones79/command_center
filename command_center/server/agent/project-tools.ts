...
export const projectToolRegistry: Record<string, ToolDefinition> = {
  ...
  "rykas.finance.snapshot": { id: "rykas.finance.snapshot", profiles: ["RYKAS_GM"], classification: "READ", sensitivity: "STANDARD", policyCategory: "RESEARCH_READ_ONLY", timeoutMs: 5000, input: emptyInput, output: rykasEnvelope,
    execute(context, _input, db) { return getOrQueueRykasTruth(context, { version: 1, operation: "FINANCIAL_SNAPSHOT", input: {} }, db); } },
  "rykas.finance.capital_plan": { id: "rykas.finance.capital_plan", profiles: ["RYKAS_GM"], classification: "READ", sensitivity: "STANDARD", policyCategory: "RESEARCH_READ_ONLY", timeoutMs: 5000, input: emptyInput, output: rykasEnvelope,
    execute(context, _input, db) { return getOrQueueRykasTruth(context, { version: 1, operation: "CAPITAL_PLAN", input: {} }, db); } },
  "rykas.inventory.replenishment_candidates": { id: "rykas.inventory.replenishment_candidates", profiles: ["RYKAS_GM"], classification: "READ", sensitivity: "STANDARD", policyCategory: "RESEARCH_READ_ONLY", timeoutMs: 5000, input: z.object({ limit: z.number().int().min(1).max(25).default(10) }).strict(), output: rykasEnvelope,
    execute(context, input, db) { return getOrQueueRykasTruth(context, { version: 1, operation: "REPLENISHMENT_CANDIDATES", input }, db); } },
  "rykas.inventory.capital_release_candidates": { id: "rykas.inventory.capital_release_candidates", profiles: ["RYKAS_GM"], classification: "READ", sensitivity: "STANDARD", policyCategory: "RESEARCH_READ_ONLY", timeoutMs: 5000, input: z.object({ limit: z.number().int().min(1).max(25).default(10) }).strict(), output: rykasEnvelope,
    execute(context, input, db) { return getOrQueueRykasTruth(context, { version: 1, operation: "CAPITAL_RELEASE_CANDIDATES", input }, db); } },
  "rykas.sale_event.evaluate": { id: "rykas.sale_event.evaluate", profiles: ["RYKAS_GM"], classification: "READ", sensitivity: "STANDARD", policyCategory: "RESEARCH_READ_ONLY", timeoutMs: 5000, input: z.object({ candidateIds: z.array(z.string().regex(/^[A-Z0-9]{10}$/)).min(1).max(25) }).strict(), output: rykasEnvelope,
    execute(context, input, db) { return getOrQueueRykasTruth(context, { version: 1, operation: "SALE_EVENT_EVALUATION", input }, db); } },
