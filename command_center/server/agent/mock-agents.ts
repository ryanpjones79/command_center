  ...
  async chooseNextWork(
    ...
    if (context.operatingMode === "LIVE_INTERNAL" && context.toolEvidence?.length) {
      ...
      if (context.profile === "RYKAS_GM") {
        ...
        const truth = output.realTruth as { stale?: boolean; purchaseExecuted?: boolean; data?: { purchaseCandidates?: Array<Record<string, unknown>>; opportunities?: Array<Record<string, unknown>>; blockers?: Array<{ summary?: string }>; financialSnapshot?: { capitalPlan?: { status?: string; safeBuyingCapacity?: number | null } } | null } } | null | undefined;
        ...
        if (candidate && truth?.stale === false && truth.purchaseExecuted === false) {
          ...
          const capitalPlan = truth.data?.financialSnapshot?.capitalPlan;
          const safeBuyingCapacity = typeof capitalPlan?.safeBuyingCapacity === "number" ? capitalPlan.safeBuyingCapacity : null;
          const expectedSpend = typeof candidate.expectedSpend === "number" ? candidate.expectedSpend : null;
          if (capitalPlan?.status !== "READY" || safeBuyingCapacity === null || expectedSpend === null) {
            return { ...plansByProfile.RYKAS_GM, disposition: "WAIT" as const, title: "Wait for deterministic Rykas capital capacity", objective: "Do not escalate a purchase until Rykas returns current safe-buying capacity and exact spend.", plannedBottleneck: "The sourcing candidate is not yet supported by a current deterministic capital plan.", ownerDecisionAfterQa: undefined };
          }
          const contextSummary = JSON.stringify({ opportunityId, product: title, supplier: candidate.supplier ?? null, quantity: candidate.recommendedUnits ?? null, totalSpend: expectedSpend, safeBuyingCapacity, expectedProfit: candidate.expectedProfit ?? null, expectedContribution: candidate.expectedMonthlyContribution ?? null, roi: candidate.roi ?? null, margin: candidate.margin ?? null, demand: candidate.estimatedMonthlyUnits ?? null, competition: candidate.sellerCount ?? null, freshness: candidate.freshness ?? null, risks: candidate.blockers ?? [], purchaseExecuted: false });
          if (expectedSpend > safeBuyingCapacity) {
            return { ...plansByProfile.RYKAS_GM, disposition: "CREATE_WORK" as const, title: `Reduce or pass: ${title}`, objective: "Present the deterministic capital conflict without inventing a reduced quantity.", acceptanceCriteria: "Ryan selects REDUCE or PASS; no order is placed and no spend above safe-buying capacity is authorized.", plannedBottleneck: `The ${expectedSpend} sourcing proposal exceeds ${safeBuyingCapacity} of safe-buying capacity.`, requiredCapability: "REPOSITORY_READ", sandboxPolicy: "READ_ONLY" as const, networkPolicy: "OFF" as const, operationalContext: `Capital-constrained decision evidence for ${opportunityId}`, ownerNeeded: true, ownerDecision: { category: "PURCHASE_INVENTORY" as const, question: `Reduce the ${title} proposal to at most $${safeBuyingCapacity.toFixed(2)}, or pass?`, context: contextSummary, recommendedChoice: "REDUCE", availableChoices: ["REDUCE", "PASS"], expectedUpside: "Preserves deterministic reserve, commitment, replenishment, and debt protections.", risk: "A reduced quantity must be recalculated by authoritative Rykas quantity logic before BUY is available.", capability: "PURCHASE", boundedPayload: { opportunityId, requestedSpend: expectedSpend, maximumSpend: safeBuyingCapacity, purchaseExecuted: false } } };
          }
          return { ...plansByProfile.RYKAS_GM, disposition: "CREATE_WORK" as const, title: `Owner purchase decision: ${title}`, objective: "Present one Rykas-authored purchase candidate that fits current deterministic safe-buying capacity.", acceptanceCriteria: "Ryan selects BUY, REDUCE, or PASS; no order is placed.", plannedBottleneck: "One real, funded Rykas purchase candidate needs owner judgment.", requiredCapability: "REPOSITORY_READ", sandboxPolicy: "READ_ONLY" as const, networkPolicy: "OFF" as const, operationalContext: `Decision evidence for ${opportunityId}`, ownerNeeded: true, ownerDecision: { category: "PURCHASE_INVENTORY" as const, question: `Buy ${String(candidate.recommendedUnits ?? "UNKNOWN")} units of ${title}?`, context: contextSummary, recommendedChoice: "BUY", availableChoices: ["BUY", "REDUCE", "PASS"], expectedUpside: `Rykas expected profit ${String(candidate.expectedProfit ?? "UNKNOWN")} and ROI ${String(candidate.roi ?? "UNKNOWN")}.`, risk: Array.isArray(candidate.blockers) && candidate.blockers.length ? candidate.blockers.join("; ") : "Inventory demand and capital remain exposed until sale.", capability: "PURCHASE", boundedPayload: { opportunityId, recommendedUnits: candidate.recommendedUnits ?? null, maximumSpend: Math.min(expectedSpend, safeBuyingCapacity), purchaseExecuted: false } } };
        }
