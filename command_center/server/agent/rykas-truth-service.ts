...
export async function getOrQueueRykasTruth(context: Context, rawRequest: unknown, db: PrismaClient, now = new Date()): Promise<RykasReadEnvelope> {
  ...
  const specialistRole = ["FINANCIAL_SNAPSHOT", "CAPITAL_PLAN"].includes(request.operation)
    ? "RYKAS_CFO_CAPITAL_STEWARD"
    : ["REPLENISHMENT_CANDIDATES", "CAPITAL_RELEASE_CANDIDATES", "SALE_EVENT_EVALUATION"].includes(request.operation)
      ? "RYKAS_SOURCING_INVENTORY_SPECIALIST"
      : "RYKAS_TRUTH_READER";
