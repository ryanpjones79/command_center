...
The later Rykas real-truth connector keeps the `rykas.operations.snapshot` ID and its legacy backlog/to-ship/listed-today fields for compatibility. When the default-off connector is activated, that tool also queues/reads a durable `RYKAS_OPERATIONS_READ` result from the outbound Windows runner. Deeper read-only tools are `rykas.sourcing.opportunities`, `rykas.sourcing.opportunity_detail`, `rykas.purchase.candidates`, and `rykas.operations.blockers`. They reuse the existing Rykas loopback Command Center and SQL marts; no Rykas tables are mirrored into RyanOS. The audited Rykas API does not expose an authoritative listing backlog or full portfolio inventory exception queue, so this phase does not invent those tools. See `RYKAS_REAL_TRUTH_CONNECTOR.md` for sources, fields, safety, activation, and rollback.
